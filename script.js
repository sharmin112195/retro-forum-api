        ...
        <div class="space-y-4 w-full">
            ...
            <div class="flex justify-between *:font-bold[&>*:not(:last-child)]:opacity-45">
                ...
                <div class="opacity-100">
                    <button id="addToList" onclick="markAsRead('${post.description}','${post.view_count}')" class="addToList btn btn-circle bg-green-500">
...
const latestPostCard = (card) =>{
    const postCard=document.getElementById("latest-post-container")
    card.forEach(cards=>{
        console.log(cards)
        const div=document.createElement("div"); 
        div.innerHTML=`
        <div class="card lg:w-96 pb-5 bg-base-100 shadow-2xl">
          ...
          <div class="p-5 lg:p-10 space-y-4 lg:space-y-5">
              <p class="opacity-50 text-start">
                  <i class="fa-solid fa-calendar-days me-2"></i>${cards.author?.posted_date || "No Publish Date"}
              </p>
              <h2 class="card-title text-start">${cards.title}</h2>
              <p class="text-start">
                  ${cards.description}
              </p>
              <div class="card-actions flex gap-5 items-center">
                  <div class="avatar">
                      ...
                      >
                          <img
                          src=${cards.profile_image}
                          />
                      </div>
                  </div>
              <div>
              <h3 class="text-start font-extrabold">${cards.author.name}</h3>
              <p class="text-start opacity-60">${cards.author?.designation || "Unknown"}</p>
          </div>
      </div>
